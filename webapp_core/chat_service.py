...
class ChatService(
    ...
    SUBJECT_FAST_DEFINITION_KEYWORDS = {
        "C_program": [
            "指针",
            "数组",
            "结构体",
    ...
    @classmethod
    def _match_fast_definition_keywords(cls, text: str) -> dict[str, list[str]]:
        normalized = cls._normalize_definition_text(text)
        if not normalized:
            return {subject_id: [] for subject_id in cls.SUBJECT_LABELS}

        matches: dict[str, list[str]] = {}
        for subject_id, keywords in cls.SUBJECT_FAST_DEFINITION_KEYWORDS.items():
            hits = []
            for keyword in keywords:
                marker = str(keyword or "").strip().lower()
                if marker and marker in normalized:
                    hits.append(keyword)
            matches[subject_id] = hits
        return matches

    def _fast_short_definition_subject_route(
        self,
        text: str,
    ) -> tuple[dict[str, Any], str] | None:
        normalized = self._normalize_definition_text(text)
        ...
        # Keep short-definition fast routing conservative.
        risky_markers = (
            "区别",
            "比较",
            "联系",
        ...
        keyword_matches = self._match_fast_definition_keywords(text)
        matched_subjects = [
            subject_id for subject_id, hits in keyword_matches.items() if hits
        ]
        if len(matched_subjects) != 1:
            return None

        subject_id = matched_subjects[0]
        hits = keyword_matches[subject_id]
        if not hits:
            return None

        scores = {item: 0.0 for item in self.SUBJECT_LABELS}
        scores[subject_id] = 1.0
        hit_preview = " / ".join(hits[:2])
        reason = (
            f"短定义题本地单学科快路径：命中 {self._subject_label(subject_id)}"
            f" 术语 {hit_preview}"
        )
        return (
            self._subject_route_from_scores(
                scores,
                confidence=1.0,
                reason=reason,
            ),
