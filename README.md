...
当前主路径是 `Flask + webapp_core 编排层 + LightRAG/OpenAI + Pydantic 结构化输出`。仓库里虽然保留了部分 `langgraph` 依赖和历史痕迹，但当前 Web 主链并不是一个以 LangGraph 为核心的运行时。新版前端使用 `React + TypeScript` 工作台展示聊天、执行流程和 Neo4j 局部知识图谱。
...
- `templates/`：旧版 Flask 前端模板，当前保留为 `/legacy`
...
旧版模板入口保留在 [http://127.0.0.1:7860/legacy](http://127.0.0.1:7860/legacy)。
...
构建产物存在时，Flask 根路径 `/` 会返回 React 前端；旧版页面仍可通过 `/legacy` 访问。
...
```bash
python utils/graph_visual_with_neo4j.py --subjects C_program operating_systems cybersec_lab
```
