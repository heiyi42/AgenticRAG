...
当前主路径是 `Flask + Web 编排层 + LightRAG/OpenAI + Pydantic 结构化输出`。仓库里虽然保留了部分 `langgraph` 依赖和历史痕迹，但当前 Web 主链并不是一个以 LangGraph 为核心的运行时。
...
- `templates/`：前端模板
- `models/`：`auto`、`instant`、`deep_search` 相关实现
...
1. 源码
   `webapp.py`、`webapp_core/`、`agenticRAG/`、`models/`、`tests/`
